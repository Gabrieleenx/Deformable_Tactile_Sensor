...
def main():
    ...
    if probe_bool:
        ...
        pose_.position.y = -0.06
        pose_.position.z = 0.02
        ...
        pose_.position.z = 0.0035
        
        pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
        probe_utility.publish_pose(pub_desired, pose_t, time=4.0)
        rospy.sleep(4.5)
        bag_process = probe_utility.start_rosbag(topics_to_record, "cpi_test", bag_directory)
        rospy.sleep(2)
        pose_.position.y = 0.04
        ...
        bag_process = probe_utility.start_rosbag(topics_to_record, "rotation_test", bag_directory)
        rospy.sleep(2)
        
        q = quaternion_from_euler(np.pi, 0, 2.0)  
        pose_.orientation.x, pose_.orientation.y, pose_.orientation.z, pose_.orientation.w = q
        pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
        probe_utility.publish_pose(pub_desired, pose_t, time=4.0)
        rospy.sleep(5)
