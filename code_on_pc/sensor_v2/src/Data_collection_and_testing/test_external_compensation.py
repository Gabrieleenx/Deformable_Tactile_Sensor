...
def main():
    ...
    if wait_for_user_input("Start test") == "y":
        probe_bool = True
        # Build Pose
        pose_reset = Pose()
        pose_reset.position.x = 0.0
        pose_reset.position.y = -0.0
        pose_reset.position.z = 0.10
        ...
        pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_reset)
        probe_utility.publish_pose(pub_desired, pose_t, time=5.0)
        rospy.sleep(5.0)
        bias_netft(sensor_nr=1)
        bias_tactile()
        rospy.sleep(1.0)
    ...
    if probe_bool:
        ...
        bag_process = probe_utility.start_rosbag(topics_to_record, "test_external_compenstation", bag_directory)
        rospy.sleep(2)
        

        q = quaternion_from_euler(np.pi, 0, 0.9*np.pi)  
        pose_.orientation.x, pose_.orientation.y, pose_.orientation.z, pose_.orientation.w = q

        pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
        probe_utility.publish_pose(pub_desired, pose_t, f_max=10, time=5)
        rospy.sleep(6)
        ...
        q = quaternion_from_euler(np.pi+np.pi/2, 0, 0)  
        pose_.orientation.x, pose_.orientation.y, pose_.orientation.z, pose_.orientation.w = q
        pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
        probe_utility.publish_pose(pub_desired, pose_t, f_max=10, time=5)
        rospy.sleep(6)
