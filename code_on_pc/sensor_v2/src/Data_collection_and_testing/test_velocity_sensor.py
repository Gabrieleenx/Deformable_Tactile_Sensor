...
class GenericSurface():
    def __init__(self, name, z_height):
        self.name = name
...
def main():
    rospy.init_node("interactive_pose_recorder_vel")
    probe_bool = False
    name_, z_height="plastic_flat", 0.0
    name_, z_height ="transparent_flat", 0.0
    name_, z_height = "paper_flat", 0.0
    name_, z_height ="wood_flat", 0.0
    name_, z_height="plasic_curved_r200", 0.005
    name_, z_height ="plasic_curved_r100", 0.012
    name_, z_height ="plasic_curved_r50", 0.016
    name_, z_height ="plasic_curved_r25", 0.022
    test_surface = GenericSurface(name=name_, z_height = z_height)
    ...
    if probe_bool:
        ...
        for normal_force in normal_force_list:
            for orientation in orientation_list:
                test_lin_vel_sensor(test_surface=test_surface, 
                                    normal_force=normal_force, 
                                    orientation=orientation,
                                    obj_pose_stamped=obj_pose_stamped,
                                    pub_desired=pub_desired)
            test_rot_vel_sensor(test_surface=test_surface, 
                                    normal_force=normal_force, 
                                    obj_pose_stamped=obj_pose_stamped,
                                    pub_desired=pub_desired)
    #rospy.spin()
...
def test_lin_vel_sensor(test_surface, normal_force, orientation, obj_pose_stamped, pub_desired):
    file_name = test_surface.name + "_" + str(normal_force) +"N_"+ str(orientation) +"_"+ "lin_vel"

    bag_directory = "" # to where data is
    topics_to_record = ["/tf", "/tf_static", "/netft_data1", "/tactile_reader/tactile_sensor", "/tactile/current_force", "/tactile_mapping/velocity"] 
    ...
    orientation_rad = orientation/180*np.pi
    q = quaternion_from_euler(np.pi, 0, orientation_rad)  
    ...
    probe_utility.publish_pose(pub_desired, pose_t, time=4.0, f_max=normal_force, t_max=1.0)
    rospy.sleep(4.5)

    bag_process = probe_utility.start_rosbag(topics_to_record, file_name, bag_directory)

    # go down to object
    pose_.position.z = test_surface.z_height
    pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
    probe_utility.publish_pose(pub_desired, pose_t, time=2.0, f_max=normal_force, t_max=1.0)
    rospy.sleep(3.5)
...
def test_rot_vel_sensor(test_surface, normal_force, obj_pose_stamped, pub_desired):
    file_name = test_surface.name + "_" + str(normal_force) +"N_"  +"_"+ "rot_vel"
    ...
    # slide
    q = quaternion_from_euler(np.pi, 0, np.pi)  
    pose_.orientation.x, pose_.orientation.y, pose_.orientation.z, pose_.orientation.w = q
    pose_t = probe_utility.pose_in_base(object_pose=obj_pose_stamped.pose, target_pose_in_object=pose_)
    probe_utility.publish_pose(pub_desired, pose_t, time=4.0, f_max=normal_force, t_max=1.0)
    rospy.sleep(5.5)
