...
class SensorConfig:
    def __init__(self, sensor_nr):
        if sensor_nr == 1:
            self.cpmm1 = 216.29 # counts per mm
            self.cpmm2 = 212.73 
            self.px1 = -7.11
            self.py1 = 0.36
            self.omega1 = 1.52
            ...
            self.cpmm1 = 185.63 # counts per mm
            self.cpmm2 = 197.04 
            self.px1 = -4.39
            self.py1 = -0.217
            self.omega1 = 1.379
...
class VelocityMapping:
    ...
    def callback(self, sensor_data):
        ...
        if self.sub_sample <= self.k:
            ...
            vel_msg.vx = sensor_vel[0, 0]/1e3
            vel_msg.vy = sensor_vel[1, 0]/1e3
            vel_msg.omega = sensor_vel[2, 0]
            self.pub.publish(vel_msg)

# ------------------- ROS Node -------------------
class TactileMappingNode:
    def __init__(self, model_path, sensor_nr):
        self.mask = make_circular_mask(32)
        
        self.frame_id = "tactile_sensor"+str(sensor_nr)
        ...
        # Publishers
        self.thickness_pub = rospy.Publisher("/tactile_mapping/thickness"+str(sensor_nr), Image, queue_size=1, tcp_nodelay=True)
        self.scalars_pub = rospy.Publisher("/tactile_mapping/wrench"+str(sensor_nr), WrenchStamped, queue_size=1, tcp_nodelay=True) # Updated
        self.hall13_pub = rospy.Publisher("/tactile_mapping/hall13"+str(sensor_nr), Float32MultiArray, queue_size=1, tcp_nodelay=True)
        self.vel_pub = rospy.Publisher("/tactile_mapping/velocity"+str(sensor_nr), TactileVelSensor, queue_size=1, tcp_nodelay=True)
        # Load model + scalers
        ...
        self.in_mean = self.in_scaler.mean_.astype(np.float32)
        self.in_scale = self.in_scaler.scale_.astype(np.float32)

        self.t_mean = self.thickness_scaler.mean_.astype(np.float32)
        self.t_scale = self.thickness_scaler.scale_.astype(np.float32)

        self.s_mean = self.scalars_scaler.mean_.astype(np.float32)
        self.s_scale = self.scalars_scaler.scale_.astype(np.float32)

        self.h13_mean = self.hall13_scaler.mean_.astype(np.float32)
        self.h13_scale = self.hall13_scaler.scale_.astype(np.float32)

        # velocity mapping
        sensor_conf = SensorConfig(sensor_nr)
        self.vel_map = VelocityMapping(sensor_config=sensor_conf,
                                       pub=self.vel_pub,
                                       sub_sample=1)

        # Subscribe to tactile sensor
        rospy.Subscriber("/tactile_reader/tactile_sensor"+str(sensor_nr), TactileSensor, self.callback, tcp_nodelay=True, queue_size=1)
        #rospy.Subscriber("/tactile_reader/tactile_sensor"+str(sensor_nr), TactileSensor, self.vel_map.callback, tcp_nodelay=True, queue_size=1)
    ...
    def callback(self, msg):
        ...
        t_pred_np = t_pred.numpy() * self.t_scale + self.t_mean
        s_pred_np = (s_pred.numpy() * self.s_scale + self.s_mean)[0]
        h13_pred_np = h13_pred.numpy() * self.h13_scale + self.h13_mean
...
if __name__ == "__main__":
    ...
    node1 = TactileMappingNode(model_path=".../model1.pth", sensor_nr=1)
    node2 = TactileMappingNode(model_path=".../model2.pth", sensor_nr=2)
    rospy.spin()
